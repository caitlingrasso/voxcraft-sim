...
VX3_Material::VX3_Material(CVX_Material *p, VX3_VoxelyzeKernel *k)
    : r(p->r), g(p->g), b(p->b), a(p->a), isTarget(p->isTarget), isPaceMaker(p->isPaceMaker), PaceMakerPeriod(p->PaceMakerPeriod),
      ...
      LockZ(p->LockZ), transparent(p->transparent), non_deformable(p->non_deformable), detachable(p->detachable), lightSensitive(p->lightSensitive), isLightSourceA(p->isLightSourceA), isLightSourceB(p->isLightSourceB), // sam
