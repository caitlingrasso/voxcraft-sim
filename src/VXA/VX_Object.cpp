...
void CVX_Object::ClearPalette()
{
	...
	CVXC_Material Empty = CVXC_Material("Erase", 0, 0, 0, 0, 0, 0);
...
CVXC_Material& CVXC_Material::operator=(const CVXC_Material& RefMat)
{
	...
	Cilia = RefMat.Cilia;
    Actuation_damper = RefMat.Actuation_damper; // caitlin
	...
	isLightSourceA = RefMat.isLightSourceA;
	isLightSourceB = RefMat.isLightSourceB;
	...
	switch (MatType){
		case SINGLE:
			if (pXML->FindElement("Mechanical")){
                ...
                if (!pXML->FindLoadElement("Actuation_damper", &Actuation_damper)) Actuation_damper = 0; // caitlin
				...
				if (!pXML->FindLoadElement("isLightSourceA", &isLightSourceA)) isLightSourceA = 0; // sam
				if (!pXML->FindLoadElement("isLightSourceB", &isLightSourceB)) isLightSourceB = 0; // sam
...
bool CVXC_Structure::WriteXML(CXML_Rip* pXML, int Compression, std::string* RetMessage)
{
	...
	for (int i=0; i<Z_Voxels; i++){
		...
		if (Compress == "ASCII_READABLE"){ for (int k=0; k < X_Voxels*Y_Voxels; k++){ WriteData.resize(X_Voxels*Y_Voxels); WriteData[k] = RawData[k]+48;}}
				...
				QByteArray QRawData = DataInString.toLatin1();
