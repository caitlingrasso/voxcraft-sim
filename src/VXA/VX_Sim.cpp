...
bool CVX_Sim::Import(CVX_Environment *pEnvIn, CMesh *pSurfMeshIn, std::string *RetMessage) {
    ...
    for (int i = 0; i < LocalVXC.GetStArraySize(); i++) { // for each voxel in the array
        ...
        if (VxcMatIndex >= 0) {
            ...
            // sam: AactivationTimeConstant
            VoxList.back()->activationTimeConstant = pEnv->pObj->Structure.GetActivationTimeConstant(VoxList.size() - 1);
            // sam: RecoveryTimeConstant
            VoxList.back()->recoveryTimeConstant = pEnv->pObj->Structure.GetRecoveryTimeConstant(VoxList.size() - 1);
            // sam: DownregulationConstant
            VoxList.back()->downregulationConstant = pEnv->pObj->Structure.GetDownregulationConstant(VoxList.size() - 1);
            // sam: DetachTime
